...
export class OpenAIContentGenerator implements ContentGenerator {
  ...
  // For Ollama: accumulate content to parse tool calls at the end
  ...
  private initializeClient(): void {
    ...
    console.log('[DEBUG] - isOllama:', this.isOllama());
    console.log('[DEBUG] - Ollama tool call parsing:', this.isOllama() ? 'enabled' : 'not needed');
  ...
  /**
   * Check if we're using Ollama vs OpenAI API
   */
   ...
   * }
   *
   * Or with <tool_call> tags:
   * {
   *   "content": "<tool_call>\n{\"name\": \"read_file\", \"arguments\": {...}}\n</tool_call>"
   * }
   */
  private parseOllamaToolCall(content: string): OpenAI.Chat.ChatCompletionMessageToolCall[] | null {
  ...
  async generateContent(
    ...
    try {
      ...
      // Ollama will return tool calls in content field, which we parse in convertToGeminiFormat
  ...
  async generateContentStream(
    ...
    try {
      ...
      // Add tools to the request
      // Ollama will return tool calls in content field, which we parse in convertStreamChunkToGeminiFormat
      if (request.config?.tools) {
        createParams.tools = await this.convertGeminiToolsToOpenAI(
          request.config.tools,
        );
  ...
  private convertToGeminiFormat(
    ...
    // Check if Ollama returned tool calls in content field instead of tool_calls array
    let parsedToolCalls: OpenAI.Chat.ChatCompletionMessageToolCall[] | null = null;
    if (this.isOllama() && choice.message.content && !choice.message.tool_calls) {
      parsedToolCalls = this.parseOllamaToolCall(choice.message.content);
    }
    ...
    const toolCalls = choice.message.tool_calls || parsedToolCalls;
    if (toolCalls) {
  ...
  private convertStreamChunkToGeminiFormat(
    ...
    if (choice) {
      ...
      // Accumulate content for Ollama tool call parsing
      if (choice.delta?.content) {
        this.streamingContent += choice.delta.content;
      ...
      if (choice.finish_reason) {
        // Check if Ollama returned tool calls in accumulated content
        let parsedToolCalls: OpenAI.Chat.ChatCompletionMessageToolCall[] | null = null;
        if (this.isOllama() && this.streamingContent && this.streamingToolCalls.size === 0) {
          parsedToolCalls = this.parseOllamaToolCall(this.streamingContent);
        ...
        // Emit tool calls from OpenAI format or parsed from Ollama content
        const toolCallsToEmit = this.streamingToolCalls.size > 0
          ? Array.from(this.streamingToolCalls.values())
          : null;

        if (toolCallsToEmit) {
          for (const accumulatedCall of toolCallsToEmit) {
          ...
          for (const toolCall of parsedToolCalls) {
            if (toolCall.function) {
              ...
              try {
                ...
                console.error('Failed to parse Ollama tool call arguments:', error);
                args = {};
              }
